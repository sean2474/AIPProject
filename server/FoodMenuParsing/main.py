...
def write_data_to_database(data: Tuple[Dict[Tuple[Any, str], dict], Dict[str, str]], database) -> bool:
    if data is None:
        return False
    else:
        data = data[0]
        try:
            ...
            def cast_to_tuple(d: dict):
                temporary_list = list()
                for key, value in d.items():
                    temporary_list.append(
                        (get_next_id(sqlite_connection),
                         '-'.join(key),
                         str(value["breakfast"]),
                         str(value["lunch"]),
                         str(value["dinner"]))
            ...
            cursor.executemany(SQL_INSERT_STATEMENT, tuple((get_next_id(sqlite_connection),) + cast_to_tuple(data)))
